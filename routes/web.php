...
Route::get('/add-job-pixel', function () {
    ...
    $jobs = JobPost::select('id','unique_id','status','job_description','dummy_job_description')->where('status',1)->where('id','>','71501')->whereNotNull('unique_id')->get();
    foreach($jobs as $job)
    {
...
Route::get('/watch/{id}', function (Request $request,$id) {
  ...
  if ($id) {
    ...
    if ($job) {
        ...
        $stats->job_id=$job->id;
        $stats->type='view';
        $stats->source=(isset($_SERVER['HTTP_REFERER']) && $_SERVER['HTTP_REFERER']!='' && $_SERVER['HTTP_REFERER']) ? str_replace('www.','',parse_url($_SERVER['HTTP_REFERER'])['host']) :null;
